<div class="d-flex justify-content-between align-items-center mb-4 mt-3">
    <h6 class="text-uppercase fw-bold text-secondary mb-0" style="letter-spacing: 0.5px;">EDIT INSTRUMEN STANDAR KERJA</h6>
</div>

<div class="row">
    <div class="col-md-10 mx-auto">
        <form action="<?= base_url('kalibrasi-internal/update/' . $instrumen->id) ?>" method="post" enctype="multipart/form-data">
            <?= csrf_field() ?>
                ...
                <div class="card-body p-4">
                    <div class="row g-3">
                            ...
                            <div class="row align-items-start g-3">
                                ...
                                <div class="col-12 col-lg-5">
                                    ...
                                    <select class="form-select border shadow-sm" name="kategori_alat" style="max-height: 140px; overflow-y: auto;">
                                        ...
                                        <option value="Pressure Gauge" <?= ($instrumen->kategori_alat ?? '') === 'Pressure Gauge' ? 'selected' : '' ?>>Pressure Gauge</option>
                                        <option value="Pressure Switch" <?= ($instrumen->kategori_alat ?? '') === 'Pressure Switch' ? 'selected' : '' ?>>Pressure Switch</option>
                                        <option value="RTD (Resistance Temperature Detector)" <?= ($instrumen->kategori_alat ?? '') === 'RTD (Resistance Temperature Detector)' ? 'selected' : '' ?>>RTD (Resistance Temperature Detector)</option>
                                        <option value="Measurement Voltage" <?= ($instrumen->kategori_alat ?? '') === 'Measurement Voltage' ? 'selected' : '' ?>>Measurement Voltage</option>
                                        <option value="Measurement Current" <?= ($instrumen->kategori_alat ?? '') === 'Measurement Current' ? 'selected' : '' ?>>Measurement Current</option>
                                        <option value="Measurement Resistance" <?= ($instrumen->kategori_alat ?? '') === 'Measurement Resistance' ? 'selected' : '' ?>>Measurement Resistance</option>
                                        <option value="Multimeter" <?= ($instrumen->kategori_alat ?? '') === 'Multimeter' ? 'selected' : '' ?>>Multimeter</option>
                                        <option value="Transducer / Transmitter" <?= ($instrumen->kategori_alat ?? '') === 'Transducer / Transmitter' ? 'selected' : '' ?>>Transducer / Transmitter</option>
                                        <option value="Thermometer" <?= ($instrumen->kategori_alat ?? '') === 'Thermometer' ? 'selected' : '' ?>>Thermometer</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        <?php
                            ...
                            function parseMultiInputInternal($data_str, $known_units) {
                                $items = array();
                                if (trim($data_str) !== '') {
                            ...
                            $interval_items = parseMultiInputInternal($instrumen->interval_kapasitas ?? '', $known_units);
                            $ketelitian_items = parseMultiInputInternal($instrumen->ketelitian ?? '', $known_units);
                            $batas_items = parseMultiInputInternal($instrumen->batas_penerimaan ?? '', $known_units);
                        ...
                        <div class="col-md-3">
                            ...
                            <div class="input-group mb-2">
                                <input type="text" class="form-control" name="interval_nilai[]" id="interval-nilai-input" value="<?= esc($interval_first['nilai']) ?>" placeholder="0-25">
                        ...
                        <div class="col-md-3">
                            ...
                            <div class="input-group mb-2">
                                <input type="text" class="form-control" name="ketelitian_nilai[]" id="ketelitian-nilai-input" value="<?= esc($ketelitian_first['nilai']) ?>" placeholder="0.001">
                        ...
                        <div class="col-md-3">
                            ...
                            <div class="input-group mb-2">
                                <input type="text" class="form-control" name="batas_nilai[]" id="batas-nilai-input" value="<?= esc($batas_first['nilai']) ?>" placeholder="0.003">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="p-1 border rounded-3 d-flex gap-1 gap-sm-2 flex-nowrap overflow-hidden" style="background-color: #f8fafc;">
                                <div class="flex-fill text-center">
                                    <input type="radio" class="btn-check" name="kondisi" id="kondisi_edit_baik_int" value="baik" <?= (empty($instrumen->kondisi) || strtolower($instrumen->kondisi) === 'baik') ? 'checked' : '' ?> required>
                                    <label class="btn btn-outline-success border-0 w-100 py-1.5 px-1 px-sm-2 rounded-3 fw-semibold d-flex align-items-center justify-content-center gap-1 text-nowrap" for="kondisi_edit_baik_int" style="font-size: 0.8rem;">
                                        <i class="bi bi-check-circle-fill"></i> Baik
                                    </label>
                                </div>
                                <div class="flex-fill text-center">
                                    <input type="radio" class="btn-check" name="kondisi" id="kondisi_edit_rusak_int" value="rusak" <?= (!empty($instrumen->kondisi) && strtolower($instrumen->kondisi) === 'rusak') ? 'checked' : '' ?>>
                                    <label class="btn btn-outline-danger border-0 w-100 py-1.5 px-1 px-sm-2 rounded-3 fw-semibold d-flex align-items-center justify-content-center gap-1 text-nowrap" for="kondisi_edit_rusak_int" style="font-size: 0.8rem;">
                                        <i class="bi bi-x-circle-fill"></i> Rusak
                                    </label>
                                </div>
                                <div class="flex-fill text-center">
                                    <input type="radio" class="btn-check" name="kondisi" id="kondisi_edit_perbaikan_int" value="perbaikan" <?= (!empty($instrumen->kondisi) && strtolower($instrumen->kondisi) === 'perbaikan') ? 'checked' : '' ?>>
                                    <label class="btn btn-outline-warning border-0 w-100 py-1.5 px-1 px-sm-2 rounded-3 fw-semibold d-flex align-items-center justify-content-center gap-1 text-nowrap" for="kondisi_edit_perbaikan_int" style="font-size: 0.8rem;">
            ...
            <div class="card shadow-sm border-0 rounded-4 mb-4">
                ...
                <div class="card-body p-4">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <label class="form-label text-dark fw-medium">Lampiran (Upload baru untuk mengganti)</label>
                            <?php if(!empty($riwayat->file_sertifikat)): ?>
                                <div class="mb-2">
                                    <a href="<?= base_url('uploads/sertifikat/' . $riwayat->file_sertifikat) ?>" target="_blank" class="badge bg-info text-decoration-none">Lihat Lampiran Saat Ini</a>
                                </div>
                            <?php endif; ?>
