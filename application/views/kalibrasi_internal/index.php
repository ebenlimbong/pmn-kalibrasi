                ...
                <div class="d-flex align-items-center justify-content-between">
                    <div>
                        <span class="text-uppercase text-muted fw-bold"
                            style="font-size: 0.75rem; letter-spacing: 0.5px;">Total Instrumen</span>
                        <h3 class="fw-bold text-dark mb-0 mt-1"><?= esc($summary['total'] ?? 0) ?> <span
                                class="fs-6 text-muted fw-normal">Unit</span></h3>
                    </div>
        ...
        <div class="card border-0 shadow-sm rounded-4 h-100 border-start border-4 border-success">
            <div class="card-body p-3">
                <div class="d-flex align-items-center justify-content-between">
                    <div>
                        <span class="text-uppercase text-muted fw-bold"
                            style="font-size: 0.75rem; letter-spacing: 0.5px;">Passed (In-Cal)</span>
                        <h3 class="fw-bold text-success mb-0 mt-1"><?= esc($summary['aktif'] ?? 0) ?> <span
                                class="fs-6 text-muted fw-normal">Unit</span></h3>
                    </div>
        ...
        <div class="card border-0 shadow-sm rounded-4 h-100 border-start border-4 border-warning">
            <div class="card-body p-3">
                <div class="d-flex align-items-center justify-content-between">
                    <div>
                        <span class="text-uppercase text-muted fw-bold"
                            style="font-size: 0.75rem; letter-spacing: 0.5px;">Jatuh Tempo Bulan Ini</span>
                        <h3 class="fw-bold text-warning mb-0 mt-1"><?= esc($summary['due_soon'] ?? 0) ?> <span
                                class="fs-6 text-muted fw-normal">Unit</span></h3>
                    </div>
        ...
        <div class="card border-0 shadow-sm rounded-4 h-100 border-start border-4 border-danger">
            <div class="card-body p-3">
                <div class="d-flex align-items-center justify-content-between">
                    <div>
                        <span class="text-uppercase text-muted fw-bold"
                            style="font-size: 0.75rem; letter-spacing: 0.5px;">Out of Spec (Overdue)</span>
                        <h3 class="fw-bold text-danger mb-0 mt-1"><?= esc($summary['overdue'] ?? 0) ?> <span
                                class="fs-6 text-muted fw-normal">Unit</span></h3>
                    </div>
                    <div class="rounded-circle bg-danger bg-opacity-10 p-3 text-danger d-flex align-items-center justify-content-center"
                        style="width: 44px; height: 44px;">
        ...
        <div class="card border-0 shadow-sm rounded-4 h-100">
            <div class="card-header bg-white pt-3 pb-2 border-0 d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex align-items-center gap-2">
                    <label for="selectTahunInternal" class="form-label mb-0 text-muted fw-semibold"
                        style="font-size: 0.78rem;">Tahun:</label>
                    <select id="selectTahunInternal" class="form-select form-select-sm shadow-sm border"
                        style="width: 95px; font-size: 0.82rem; font-weight: 600;"
                        onchange="location.href='?tahun=' + this.value;">
    ...
    <div class="card-body p-4">
        <div class="row align-items-center mb-3 g-2">
            ...
            <div class="col-12 col-md-5 col-lg-4 d-flex gap-2">
                <select id="customLength" class="form-select form-select-sm shadow-sm border-0"
                ...
                <div class="input-group input-group-sm shadow-sm border-0 bg-white rounded flex-grow-1">
                    ...
                    <input type="text" id="customSearch" class="form-control border-0 shadow-none"
                        placeholder="Cari...">
                </div>
            </div>
            ...
            <div class="col-12 col-md-auto d-flex gap-2 mt-2 mt-md-0">
                <input type="date" id="customDateFilter"
                    class="form-control form-control-sm shadow-sm border-0 flex-grow-1" style="min-width: 140px;"
                    title="Pilih tanggal">
                <button id="clearFilterBtn" class="btn btn-sm text-white shadow-sm text-nowrap px-3"
                    style="background-color: #3b5998;">Clear Filter</button>
            </div>

            <!-- Third Group (Refresh & Tambah) -->
            <div class="col-12 col-md-auto ms-md-auto mt-2 mt-md-0 d-flex justify-content-end gap-2">
                <button id="refreshBtn" class="btn btn-sm text-white shadow-sm text-nowrap px-3"
                    style="background-color: #fbb03b;"><i class="bi bi-arrow-clockwise"></i> Refresh</button>
                <a href="<?= base_url('kalibrasi-internal/create') ?>"
                    class="btn btn-sm btn-primary shadow-sm px-3 fw-medium"
                    style="background-color: #3b5998; border: none;">+ Tambah Data</a>
            </div>
        </div>

        <table id="tabelInstrumen" class="table table-hover align-middle mb-0 table-sm text-nowrap text-center"
                        ...
                        <tr>
                            <td>
                                <?php if (!empty($item->foto_alat)): ?>
                                    <img src="<?= base_url('uploads/instrumen/' . esc($item->foto_alat)) ?>" alt="Foto"
                                        class="img-thumbnail" style="width: 50px; height: 50px; object-fit: cover;">
                                <?php else: ?>
                                    <div class="bg-light d-flex align-items-center justify-content-center text-muted border rounded"
                                        style="width: 50px; height: 50px; font-size: 0.8rem;">No Img</div>
                                <?php endif; ?>
                            </td>
                            <td class="text-start"><?= esc($item->nama_instrumen ?? '-') ?></td>
                            <td><?= esc($item->seksi_pemakai ?? '-') ?></td>
                            <td><span
                                    class="badge bg-info bg-opacity-10 text-dark border border-info border-opacity-25 px-2 py-1"
                                    style="font-size: 0.78rem;"><?= esc($item->kategori_alat ?? '-') ?></span></td>
                            <td class="fw-medium text-dark"><?= esc($item->nomor_identifikasi) ?></td>
                            <td><span class="badge bg-light text-secondary border rounded-pill px-2 py-1 fw-medium"
                                    style="font-size: 0.78rem; background-color: #f8f9fa !important; border-color: #dee2e6 !important;"><i
                                        class="bi bi-clock-history me-1 opacity-75"></i><?= esc(hitung_umur_instrumen($item->tanggal_mulai_digunakan ?? '')) ?></span>
                            </td>
                            ...
                            <td class="text-center">
                                <div class="d-flex justify-content-center gap-2">
                                    <a href="<?= base_url('kalibrasi-internal/detail/' . $item->id) ?>"
                                        class="btn btn-info text-white shadow-sm p-2 rounded-3 d-flex align-items-center justify-content-center border-0"
                                        title="Detail" style="width: 35px; height: 35px;">
                                        <i class="bi bi-eye" style="font-size: 1.1rem;"></i>
                                    </a>
                                    <a href="<?= base_url('kalibrasi-internal/edit/' . $item->id) ?>"
                                        class="btn btn-primary text-white shadow-sm p-2 rounded-3 d-flex align-items-center justify-content-center border-0"
                                        title="Edit" style="width: 35px; height: 35px;">
                                        <i class="bi bi-pencil" style="font-size: 1.1rem;"></i>
                                    </a>
                                    <form action="<?= base_url('kalibrasi-internal/delete/' . $item->id) ?>" method="post"
                                        class="d-inline m-0"
                                        onsubmit="return confirm('Apakah Anda yakin ingin menghapus instrumen ini?');">
                                        <?= csrf_field() ?>
                                        <button type="submit"
                                            class="btn btn-danger text-white shadow-sm p-2 rounded-3 d-flex align-items-center justify-content-center border-0"
                                            title="Hapus" style="width: 35px; height: 35px;">
